...
class SmartConfig_Export(bpy.types.Operator, ExportHelper):
    ...
    def draw(self, context):
        ...
        box = layout.box()
        box.label(text = 'Export Settings :', icon='NONE')
...
def smartconfig_export_config(filepath, context, include_startup_file, include_bookmarks, include_users_prefs, include_presets, include_config_folders):
    ...
    if chk_p==0:
        ...
        total=len(bpy.context.preferences.addons.keys())
        nb=0
        ...
        if include_bookmarks:
            try:
                ...
                print("Smart Config warning : No bookmark(s) file found")
        # copy userprefs file
        if include_users_prefs:
            try:
                path_s=os.path.join(os.path.join(user_path, "config"),"userpref.blend")
                path_d=os.path.join(tempdir, "userpref.blend")
                shutil.copy2(path_s, path_d)
            except:
                print('Smart Config warning : No userpref Found...')
        # copy presets folder
        if include_presets:
            try:
                ...
                print('Smart Config warning : No Preset Found...')
        # copy config folders
        if include_config_folders:
            config=os.path.join(user_path, "config")
            config_d=os.path.join(tempdir, "config")
            for file in os.listdir(config):
                if os.path.isdir(os.path.join(config,file)):
        ...
        #error reporting
        if len(wrong)!=0:
            if addon_preferences.errors_report:
